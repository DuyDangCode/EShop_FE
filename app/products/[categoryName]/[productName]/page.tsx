...
export default function ProductDetailPage({ params }: ProductDetailPageProps) {
  ...
  function addProductToCard(productId: string, quantity: number) {
    const addProductToCartPromise = apiInstance.post(apiHelper.cart(), {
      product: {
        productId: productId,
        productQuantity: quantity,
      },
    })
    promiseToast(addProductToCartPromise, 'Add successful', 'Fail')
  }
            ...
            <div className='flex flex-col flex-[2] h-fit gap-3 m-5 '>
              ...
              <div className='flex md:flex-row flex-col gap-5 md:w-1/2 w-full h-[100px]'>
                ...
                <Button
                  color='red'
                  onClick={() => {
                    addProductToCard(data?.metadata?._id, quantity)
                  }}
                  className='flex-[1]'
                >
                  Add to cart
